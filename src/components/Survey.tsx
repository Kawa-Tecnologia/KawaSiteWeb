...
const SurveyForm: React.FC<SurveyFormProps> = ({ email, providerName, serviceId }) => {
  ...
  const handleStarClick = (category: string, rating: number) => {
    // Verifica se o rating selecionado é igual ao rating atual, se sim, define o valor como 0
  ...
  return (
    <div className="container-survey">
      ...
      <form onSubmit={handleSubmit}>
        ...
        {/* Repita para os outros aspectos de avaliação */}
        ...
        <div className="star-rating horizontal">
          {[1, 2, 3, 4, 5].map((rating) => (
            <label key={rating}>
              ...
              <StarIcon
                className="star"
                color={rating <= formData.technical_knowledge ? 'primary' : 'action'}
              />
            </label>
          ))}
        </div>
